...
func Run(ctx context.Context) error {
	...
	twoFARepo := repository.NewTwoFARepository(redisClient, cfg.AppEnv)                                                             // 2FA challenge、失败限流和防重放存储
	...
	userService := service.NewUserService(userRepo, roleRepo, ivService, twoFARepo, jwtManager, cfg.PasswordCryptoKey, cfg.AppName) // 用户业务层：登录、鉴权、密码管理
