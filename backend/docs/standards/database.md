...
PostgreSQL 17.9 is the source of truth for business data. Schema, queries, and migrations must be explicit and reviewable.

## Schema Naming

- Use lowercase snake_case for tables and columns.
- Use plural table names: `users`, `orders`, `login_logs`.
- Primary keys should be `id BIGINT GENERATED BY DEFAULT AS IDENTITY` unless there is a strong reason otherwise.
...
- Use PostgreSQL placeholders such as `$1`, `$2`, and `$3`.
