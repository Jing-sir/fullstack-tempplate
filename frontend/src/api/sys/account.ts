...
class SysAccountApi extends Api {
    ...
    async sysUserInfo(params: { userId: string }): Promise<SysUserInfoResult> {
        return this.api.get<SysUserInfoResult, SysUserInfoResult>(
            `/admin-users/detail/${params.userId}`,
        )
    }
    ...
    async setSysUserResetSecret(params: SysUserResetSecretParams): Promise<void> {
        await this.api.post<void, void, SysUserResetSecretParams>('/admin-users/reset-2fa', params)
    }
}
