...
class SysRoleApi extends Api {
    ...
    async menuList(): Promise<RoleMenuItem[]> {
        const result = await this.api.get<Partial<RoleMenuItem>[], Partial<RoleMenuItem>[]>(
            '/menus',
        );
        return result.map(normalizeMenuItem);
    }

    /** 角色列表（用于下拉选择 / 角色管理列表） */
    async sysRoleList(): Promise<SystemRoleItem[]> {
        const result = await this.api.get<Array<Record<string, unknown>>, Array<Record<string, unknown>>>('/roles');
        // 后端返回 {roles: [...]} 或直接数组
        const list = Array.isArray(result) ? result : ((result as Record<string, unknown>).roles as Array<Record<string, unknown>>) ?? [];
        return list.map((r) => ({
            roleId: String(r.id ?? ''),
    ...
    async sysRoleMenuList(): Promise<TreeDataType[]> {
        const result = await this.api.get<Array<Record<string, unknown>>, Array<Record<string, unknown>>>('/admin/menus');
    ...
    async sysRoleInfo(params: { roleId: string }): Promise<RoleInfoResult> {
        return this.api.get<RoleInfoResult, RoleInfoResult>(`/roles/${params.roleId}/info`);
    }
