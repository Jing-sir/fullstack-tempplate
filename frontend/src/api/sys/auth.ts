...
class SysAuthApi extends Api {
    ...
    async validateGoogle(params: { googleCode?: string; code?: string }): Promise<SysUserLoginResult> {
    ...
    // getMe 调用 /userInfo 接口，返回当前用户信息和权限列表
    async getMe(): Promise<MeResult> {
        return this.api.get<MeResult, MeResult>('/userInfo');
    }
