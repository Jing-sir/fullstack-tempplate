...
class UserSecurityApi extends Api {
    ...
    async verifyGoogleCodeAndBind(params: { code: string }): Promise<void> {
        await this.api.post<void, void, { code: string }>('/user/2fa/verify', params);
    }

    async validateGoogleCode(params: { googleCode: string }): Promise<void> {
        await this.verifyGoogleCodeAndBind({ code: params.googleCode });
    }
}
